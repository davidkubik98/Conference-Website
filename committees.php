  ...
  <section class="section-margin">
    <div class="container">
      <div class="section-intro text-center pb-90px">
        <p class="section-intro__title">Choose the Sub Commitee</p>
        <h2 class="primary-text">Find A Sub Committee</h2>
...
try{
  require './config.php';
    ...
    $query = $pdo->query("SELECT committee_name FROM committee");
?>
    <form method="post">
    <select name="com">
...
if(isset($_POST['submit'])){
  $name = @$_POST['com'];
  $query = "SELECT member FROM committee_member WHERE
  committee_id=?";
  $sql = "SELECT committee_id FROM committee WHERE
  committee_name=?";
  $result = $pdo->prepare($sql);
  $result->execute([$name]);
  $name_id=$result->fetch();
  $id = $name_id['committee_id'];
  $fnlRslt = $pdo->prepare($query);
  $fnlRslt->execute([$id]);

  if($fnlRslt->rowCount() > 0){ ?>
    <h2> Committee Members</h2> <?php
      echo "<table>";
          echo "<tr>";
              echo "<th>Name</th>";
          echo "</tr>";
      foreach($row = $fnlRslt->fetchAll() as $entry){
          echo "<tr>";
              echo "<td>" . $entry['member'] . "</td>";
          echo "</tr>";
      }
      echo "</table>";
      // Free result set
