...
<body>
  ...
  <section class="section-margin">
    <div class="container">
      <div class="section-intro text-center pb-90px">
        <p class="section-intro__title">Enter the Sponsor ID</p>
        <h2 class="primary-text">Find A Job</h2>
  ...
  <body>
	...
	<?php
	    ...
	    try{
				 $id = $_POST['id'] ;
				 $sql = "SELECT job_title, city, province, rate FROM ad where sponsor_id=?";
			   $result = $pdo->prepare($sql);
				 $result->execute([$id]);
	        if($result->rowCount() > 0){ ?>
						<h2> Jobs Offered</h2> <?php
	            echo "<table>";
	                echo "<tr>";
                      ...
                      echo "<th>Rate</th>";
	                echo "</tr>";
	            foreach($row = $result->fetchAll() as $entry){
	                echo "<tr>";
	                    echo "<td>" . $entry['job_title'] . "</td>";
                      echo "<td>" . $entry['city'] . "</td>";
                      echo "<td>" . $entry['province'] . "</td>";
                      echo "<td>" . $entry['rate'] . "</td>";
	                echo "</tr>";
	            }
	            echo "</table>";
	            // Free result set
